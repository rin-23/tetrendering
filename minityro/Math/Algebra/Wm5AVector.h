...
public:
    ...
    // AVector (Float3&& tuple);
    AVector (const Vector3f& vec);
    AVector (Vector3f&& vec);
    ~AVector ();
    ...
    AVector operator- (const AVector& vec) const;
    AVector operator+ (AVector&& vec) const;
    AVector operator- (AVector&& vec) const;
    ...
    float Dot (AVector&& vec) const;
    float Normalize (const float epsilon = Mathf::ZERO_TOLERANCE);
    AVector GetNormalized (const float epsilon = Mathf::ZERO_TOLERANCE) const;
    AVector Cross (const AVector& vec) const;
    AVector Cross (AVector&& vec) const;
    AVector UnitCross (const AVector& vec) const;
    AVector UnitCross (AVector&& vec) const;
    ...
    static const AVector UNIT_NEGATIVE_Z;  // (0,0,-1,0)
