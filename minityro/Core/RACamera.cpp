...
//        mGyroscopeRotationEnabled = false;
        ...
        mViewport = Eigen::Vector4i(0,0,0,0);
        mIsOrtho = true;
        mAspect = 1.0f;
        UpdateProjectionMatrix();
    }
    
    Camera::Camera(const Wm5::APoint& position, const Eigen::Vector4i& viewport, float aspect, bool isOrtho)
    {
        mViewMatrix.MakeIdentity();
        mPosition = position;
        mOrientation = HQuaternion::IDENTITY;
//        mGyroscopeRotationEnabled = false;
    ...
    Camera::Camera(const Wm5::APoint& position, const Wm5::APoint& target, const Eigen::Vector4i& viewport, float aspect, bool isOrtho)
    {
        mViewMatrix.MakeIdentity();
        mPosition = position;
        mOrientation = HQuaternion::IDENTITY;
...
//        mGyroscopeRotationEnabled = false;
    ...
    void Camera::SetViewport(const Eigen::Vector4i& viewport)
    {
        mViewport = viewport;
    }
    ...
    Eigen::Matrix4f Camera::GetEigenViewMatrix() const 
    {   
        Wm5::HMatrix mm = GetViewMatrix();
        Eigen::Matrix4f e1 = Eigen::Map<Eigen::Matrix4f>(mm.mEntry);
        e1.transposeInPlace();
        return e1;
    }
    
    Eigen::Matrix4f Camera::GetEigenProjMatrix() const
    {
        Wm5::HMatrix mm = GetProjectionMatrix();
        Eigen::Matrix4f e2 = Eigen::Map<Eigen::Matrix4f>(mm.mEntry);
        e2.transposeInPlace();

        return e2;
    }
    Eigen::Matrix3f Camera::GetEigenNormalMatrix() const 
    {
        Wm5::HMatrix viewMatrix = GetViewMatrix();
        // Eigen::HMatrix projectionMatrix = camera->GetProjectionMatrix();
        // Eigen::HMatrix modelViewMatrix = viewMatrix;// * WorldTransform.Matrix();
        // Eigen::HMatrix modelViewProjectionMatrix =  camera->GetProjectionMatrix() * modelViewMatrix;
        Wm5::Matrix3f normalMatrix = viewMatrix.GetNormalMatrix();
        Eigen::Matrix3f e2 = Eigen::Map<Eigen::Matrix3f>(normalMatrix);
        e2.transposeInPlace();
        return e2;
    }

    
    Eigen::Vector4f Camera::GetEigenViewport() const
    {
