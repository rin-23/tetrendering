...
namespace tyro 
{
	...
	void ES2Renderer::RenderVisibleSet(const VisibleSet* visualSet, const Camera* camera)
	{
		...
		for (int i = 0; i < visualSet->GetNumVisible(); ++i)
		{
			...
			//{
				...
				Vector3f cameraPos = camera->GetViewMatrix() * APoint(renderable->WorldBoundBox.GetCenter());
	...
	void ES2Renderer::SetBufferTextures(const ES2VisualEffect* ve) const
	{
		...
		{	
			...
			// assert(i+1 == texid);
			glActiveTexture(GL_TEXTURE0+i);  //If rendering is supper slow make sure that GL_TEXTUREi has i == GetTextureID()
