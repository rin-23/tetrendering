...
//        static ES2RenderableSPtr Create(ES2PrimitiveType type);
        ...
        inline void ComputeExtremesL(int numVectors, size_t stride, const void* vectors);
    ...
    public:
        ...
        Eigen::AlignedBox<float,3> LocalBoundBox;
    ...
    inline void ES2Renderable::ComputeExtremesL(int numVectors, size_t stride, const void* vectors)
    {
        ...
        {
            const Eigen::Vector3f& vec = *(Eigen::Vector3f*)((const char*)vectors + j*stride);
            if (j == 0)
            {
                vmin = vec;
                vmax = vmin;
        ...
        LocalBoundBox = Eigen::AlignedBox<float,3>(vmin,vmax);
    }
    /****INLINE FUNCTIONS****/
