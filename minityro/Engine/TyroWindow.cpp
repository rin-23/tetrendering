...
#include "../External/stb_folder/stb/stb_image.h"
...
int Window::Init(int w, int h)
{
    ...
    if (GLEW_VERSION_3_2) 
        RA_LOG_INFO("Yay! OpenGL 3.3 is supported!");
    ...
    std::string png_file = std::string(MY_RESOURCE_FILE) + std::string("/data/pen_cursor.png");
    int cols,rows,n;
    unsigned char *pixels1 = ::stbi_load(png_file.c_str(), &cols, &rows, &n, 4);
    GLFWimage image1;
    image1.width = rows;
    image1.height = cols;
    image1.pixels = pixels1;
    m_gl_pen_cursor = glfwCreateCursor( &image1, 0, 0 );

    png_file = std::string(MY_RESOURCE_FILE) + std::string("/data/x_cursor.png");
    //int cols,rows,n;
    unsigned char *pixels2 = ::stbi_load(png_file.c_str(), &cols, &rows, &n, 4);
    GLFWimage image2;
    image2.width = rows;
    image2.height = cols;
    image2.pixels = pixels2;
    m_gl_x_cursor = glfwCreateCursor( &image2, 0, 0 );
